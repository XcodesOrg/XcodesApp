...
extension URL {
    static let downloadableRuntimes =
...
public struct RuntimeService {
    ...
    /// Loops through `/Library/Developer/CoreSimulator/images/images.plist` which contains a list of downloaded
    /// Simuator Runtimes
    /// This is different then using `simctl` (`installedRuntimes()`) which only returns the installed runtimes for the
    /// selected xcode version.
    public func localInstalledRuntimes() async throws -> [CoreSimulatorImage] {
        guard let path = Path("/Library/Developer/CoreSimulator/images/images.plist")
        else { throw MessageError("Could not find images.plist for CoreSimulators") }
        guard let infoPlistData = FileManager.default.contents(atPath: path.string)
        else { throw MessageError("Could not get data from \(path.string)") }

        do {
            let infoPlist: CoreSimulatorPlist = try PropertyListDecoder().decode(
                CoreSimulatorPlist.self,
                from: infoPlistData
            )
