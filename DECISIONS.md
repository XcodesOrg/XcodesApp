...
To begin, we will intentionally not attempt to share code between Rhodon and Rhodon. In the future, once we have a better idea of the two tools' functionality, we can revisit this decision. An example of code that could be shared are the two AppleAPI libraries which will likely be very similar.

While the intent of RhodonKit library was to potentially reuse it in a GUI context, it still makes a lot of assumptions about how the UI works that would prevent that happening immediately. As we reuse that code (by copying and pasting) and tweak it to work in Rhodon, we may end up with something that can work in both contexts. 

## Asynchrony

Rhodon uses Combine to model asynchronous work. This is different than Rhodon, which uses PromiseKit because it began prior to Combine's existence. This means that there is a migration of the existing code that has to happen, but the result is easier to use with a SwiftUI app.

## Dependency Injection

Rhodon used Point Free's Environment type, and I'm happy with how that turned out. It looks a lot simpler to implement and grow with a codebase, but still allows setting up test double for tests.
...
The helper no longer uses the private audit token API. It validates the connecting client by asking Security.framework for the `SecCode` associated with the `NSXPCConnection` process identifier, then checks the expected bundle identifier, minimum version, Apple generic anchor, team identifier, hardened runtime, and problematic entitlements. This keeps the helper pure Swift and avoids shipping an Objective-C bridge for private API access. PID-based validation is deliberately limited to the listener acceptance path and is immediately followed by code-signing requirements, which is the tradeoff discussed in Apple DTS guidance for this style of privileged helper.

The helper treats Xcode paths as untrusted input. Commands that operate on Xcode now accept only an absolute `.app` bundle with `CFBundleIdentifier` `com.apple.dt.Xcode`, a `Contents/Developer` directory, an executable `Contents/Developer/usr/bin/xcodebuild`, and a valid Apple code signature for the Xcode bundle. The helper only runs documented command paths and applies a timeout so failed child processes do not block the XPC call indefinitely.
...
We're familiar with using GitHub releases to distribute pre-built, code signed and notarized versions of `Rhodon` via direct download and Homebrew. Ideally we could use GitHub releases here too with an update mechanism more suitable for an app bundle. For distribution outside the Mac App Store, the most popular choice for updates is [Sparkle](https://sparkle-project.org). The v2 branch has been in beta for a long time, but since Rhodon isn't (currently) sandboxed, we can use the production-ready v1 releases.
