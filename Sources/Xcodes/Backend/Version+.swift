...
public extension Version {
    ...
    ///
    /// We need a way to determine if two Xcode versions are the same without always having full information, and
    ...
    func isEquivalent(to other: Version) -> Bool {
        // If we don't have build metadata identifiers for both Versions, compare major, minor, patch and prerelease
        // identifiers.
        if buildMetadataIdentifiers.isEmpty || other.buildMetadataIdentifiers.isEmpty {
            ...
            major == other.major &&
                minor == other.minor &&
                patch == other.patch &&
                buildMetadataIdentifiers.map { $0.lowercased() } == other.buildMetadataIdentifiers
                .map { $0.lowercased() }
        }
    }
    ...
    var isPrerelease: Bool {
        prereleaseIdentifiers.isEmpty == false
    }

    var isNotPrerelease: Bool {
        prereleaseIdentifiers.isEmpty == true
    }
}
