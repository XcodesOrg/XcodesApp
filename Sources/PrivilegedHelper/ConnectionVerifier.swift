...
class ConnectionVerifier {
    ...
    private static func verifyHardenedRuntimeAndProblematicEntitlements(secStaticCode: SecStaticCode) -> Bool {
        var signingInformationOptional: CFDictionary?
        if
            SecCodeCopySigningInformation(
                secStaticCode,
                SecCSFlags(rawValue: kSecCSDynamicInformation),
                &signingInformationOptional
            ) != errSecSuccess {
        ...
        // Skip this check for debug builds because they'll have the get-task-allow entitlement
            ...
            for problematicEntitlement in problematicEntitlements {
                if let presentEntitlement = entitlements.object(forKey: problematicEntitlement) as? Int {
                    if presentEntitlement == 1 {
                        Logger.connectionVerifier
                            .error("The sender has \(problematicEntitlement) entitlement set to true")
                        return false
                    }
                }
            }
    ...
    private static func verifyWithRequirementString(secCode: SecCode) -> Bool {
        ...
        // RequirementLang/RequirementLang.html#//apple_ref/doc/uid/TP40005929-CH5-SW1
        let requirementString =
            // swiftlint:disable:next line_length
            "identifier \"\(clientBundleID)\" and info [CFBundleShortVersionString] >= \"1.0.0\" and anchor apple generic and certificate leaf[subject.OU] = \"\(subjectOrganizationalUnit)\"" as NSString

        var secRequirement: SecRequirement?
        if
            SecRequirementCreateWithString(requirementString as CFString, SecCSFlags(rawValue: 0), &secRequirement) !=
            errSecSuccess {
    ...
    static func isValid(connection: NSXPCConnection) -> Bool {
