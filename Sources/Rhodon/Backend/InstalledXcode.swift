...
public struct InstalledXcode: Equatable, @unchecked Sendable {
    ...
    public init?(path: Path) {
        ...
        // Installed betas don't include the beta number anywhere, so try to parse it from the filename or fall back to
        // simply "beta"
        var prereleaseIdentifiers = bundleVersion.prereleaseIdentifiers
        if
            let filenameVersion = Version(path.basename(dropExtension: true).replacingOccurrences(
                of: "Xcode-",
                with: ""
            )) {
        ...
        let archsString = try? RhodonKit.current.shell.archs(path.url.appending(path: "Contents/MacOS/Xcode")).out
