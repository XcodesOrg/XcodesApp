...
public enum InstallationError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case let .unsupportedFileFormat(fileExtension):
            "Rhodon doesn't (yet) support installing Xcode from the \(fileExtension) file format."
        case .missingSudoerPassword:
        ...
        case let .postInstallStepsNotPerformed(version, helperInstallState):
            ...
            case .notInstalled, .unknown:
                // swiftlint:disable:next line_length
                "Installation was completed, but some post-install steps weren't performed automatically. Rhodon performs these steps with a privileged helper, which appears to not be installed. You can install it from Preferences > Advanced.\n\nThese steps will be performed when you first launch Xcode \(version.appleDescription)."
            }
        }
    }
}
